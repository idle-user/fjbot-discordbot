...
class Admin(commands.Cog):
	...
	async def add_discord_command(self, ctx, command, *, response):
		...
		if res['success']:
			embed = quickembed.success(desc='Command `{}` updated'.format(command), use=user)
		else:
	...
	async def mute_member(self, ctx, member: discord.Member):
		...
		if not role:
			embed = quickembed.error(desc='`Muted` role does not exist'.format(ctx.author), user=user)
		elif role not in member.roles:
			await member.add_roles(role)
			embed = quickembed.success(desc='Muted {}'.format(ctx.author), user=user)
		else:
			embed = quickembed.error(desc='{} is already muted'.format(ctx.author), user=user)
	...
	async def unmute_member(self, ctx, member: discord.Member):
		...
		if not role:
			...
			embed = quickembed.error(desc='{} is already unmuted'.format(ctx.author), user=user)
		await ctx.send(embed=embed)
